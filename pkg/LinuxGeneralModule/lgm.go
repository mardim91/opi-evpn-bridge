...
func set_up_bridge(LB *infradb.LogicalBridge) bool {
	...
	if !reflect.ValueOf(LB.Spec.Vni).IsZero() {
		...
		if err != nil {
			fmt.Printf("LGM: Failed to get link information for %s: %v\n", br_tenant, err)
                	return false
		}
		...
		if err := nlink.LinkAdd(ctx, vxlan); err != nil {
			fmt.Printf("LGM: Failed to create Vxlan linki %s: %v\n", link, err)
			return false
		}
		...
		if err = nlink.LinkSetMaster(ctx, vxlan, brIntf); err != nil {
			fmt.Printf("LGM: Failed to add Vxlan %s to bridge %s: %v\n",link, br_tenant, err)
			return false
		}
		// Example: ip link set vxlan-<LB-vlan-id> up
		if err = nlink.LinkSetUp(ctx, vxlan); err != nil {
			fmt.Printf("LGM: Failed to up Vxlan link %s: %v\n", link, err)
			return false
		}
		...
		if err = nlink.BridgeVlanAdd(ctx, vxlan, uint16(LB.Spec.VlanId), true, true, false, false); err != nil {
			fmt.Printf("LGM: Failed to add vlan to bridge %s: %v\n",br_tenant, err)
			return false
		}
		if err = nlink.LinkSetBrNeighSuppress(ctx, vxlan, true); err != nil {
			fmt.Printf("LGM: Failed to add bridge %s neigh_suppress: %v\n", vxlan, err)
			return false
		}
		/*
...
func set_up_vrf(VRF *infradb.Vrf) (string, bool) {
	...
	if (link_err != nil){
		fmt.Printf("LGM : Link %s not found\n",VRF.Name)
		return "", false
	}

	linkmtu_err := nlink.LinkSetMTU(ctx,link,ip_mtu)
	if (linkmtu_err != nil){
		fmt.Printf("LGM : Unable to set MTU to link %s \n",VRF.Name)
                return "", false
        }
	...
	if !reflect.ValueOf(VRF.Spec.Vni).IsZero() {
		...
		br_err := nlink.LinkAdd(ctx,&netlink.Bridge{                       
	        ...
	        if br_err != nil {
			fmt.Printf("LGM : Error in added bridge port\n")
                	return "", false
	        }
		...
		linkmtu_err := nlink.LinkSetMTU(ctx,link_br,ip_mtu)
       		if (linkmtu_err != nil){
			fmt.Printf("LGM : Unable to set MTU to link br-%s \n",VRF.Name)
	                return "", false
	        }
		
		link_master, err_master := nlink.LinkByName(ctx,VRF.Name)
	        if err_master != nil {
			fmt.Printf("LGM : Error in getting the %s\n",VRF.Name)
                	return "", false
	        }

		err :=nlink.LinkSetMaster(ctx,link_br, link_master)
	        if err != nil {
			fmt.Printf("LGM : Unable to set the master to br-%s link",VRF.Name)
                	return "", false
	        }
		...
		vxlan_err := nlink.LinkAdd(ctx,&netlink.Vxlan{
			LinkAttrs: netlink.LinkAttrs{Name: "vxlan-"+VRF.Name,MTU :ip_mtu,},VxlanId: int(*VRF.Spec.Vni),SrcAddr : Src_vtep, Learning : false, Proxy : true,Port : 4789,})
                if vxlan_err != nil {
			fmt.Printf("LGM : Error in added vxlan port\n")
                        return "", false
                }
		...
		err=nlink.LinkSetMaster(ctx,link_vxlan, link_br)
                if err != nil {
			fmt.Printf("LGM : Unable to set the master to vxlan-%s link",VRF.Name)
                        return "", false
                }
...
func set_up_svi(SVI *infradb.Svi) (string, bool) {
	...
	if err = nlink.BridgeVlanAdd(ctx, brIntf, uint16(vid), true, false, false, false); err != nil {
		fmt.Printf("LGM : Failed to add VLAN %d to bridge interface %s: %v\n", vid, br_tenant, err)
                 return "", false
        }
	/*
	...
	vlanLink := &netlink.Vlan{LinkAttrs: netlink.LinkAttrs{Name: link_svi, ParentIndex: brIntf.Attrs().Index,},VlanId: int(vid),}
		...
		fmt.Printf("LGM : Failed to add VLAN sub-interface %s: %v\n", link_svi, err)
                return "", false
        }
	/*
	...
	fmt.Printf("LGM Executed : ip link add link %s name %s type vlan id %s\n", br_tenant, link_svi, vid)
	if err = nlink.LinkSetHardwareAddr(ctx, vlanLink, *SVI.Spec.MacAddress); err != nil {
		fmt.Printf("LGM : Failed to set link %s: %v\n", vlanLink, err)
		return "", false
	}
	/*
	...
	fmt.Printf("LGM Executed : ip link set %s address %s\n", link_svi, MacAddress)
	vrfIntf, err := nlink.LinkByName(ctx, path.Base(SVI.Spec.Vrf))
		...
		fmt.Printf("LGM : Failed to set master for %s: %v\n", vlanLink, err)
                return "", false
        }
        if err = nlink.LinkSetUp(ctx, vlanLink); err != nil {
		fmt.Printf("LGM : Failed to set up link for %s: %v\n", vlanLink, err)
                return "", false
        }
        if err = nlink.LinkSetMTU(ctx, vlanLink, ip_mtu); err != nil {
		fmt.Printf("LGM : Failed to set MTU for %s: %v\n", vlanLink, err)
                return "", false
        }
	/*
	...
	fmt.Printf("LGM Executed :  ip link set %s master %s up mtu %s\n", link_svi, path.Base(SVI.Spec.Vrf), ip_mtu)
	...
	for _,ip_intf := range SVI.Spec.GatewayIPs {
		...
		if err:= nlink.AddrAdd(ctx, vlanLink, addr); err != nil {
			fmt.Printf("LGM: Failed to add ip address %s to %s: %v\n", addr)
			return "", false
		}
		/*
...
func tear_down_vrf(VRF *infradb.Vrf) bool {
	...
	link,err1 := nlink.LinkByName(ctx,VRF.Name)
        if (err1 != nil){
		fmt.Printf("LGM : Link %s not found %+v\n",VRF.Name,err1)
                return true
        }
	...
	if !reflect.ValueOf(VRF.Spec.Vni).IsZero() {
		...
		link_vxlan,link_err := nlink.LinkByName(ctx,"vxlan-"+VRF.Name)
        	if (link_err != nil){
			fmt.Printf("LGM : Link vxlan-%s not found %+v\n",VRF.Name,link_err)
                	return false
        	}
		...
		link_br,linkbr_err := nlink.LinkByName(ctx,"br-"+VRF.Name)
                if (linkbr_err != nil){
			fmt.Printf("LGM : Link br-%s not found %+v\n",VRF.Name,linkbr_err)
                        return false
                }
...
func tear_down_svi(SVI *infradb.Svi) bool {
	...
	Intf, err := nlink.LinkByName(ctx, link_svi)
        if err != nil {
		fmt.Printf("LGM : Failed to get link %s: %v\n", link_svi)
                return true
        }
	/*
	...
	if err = nlink.LinkDel(ctx, Intf); err != nil {
		fmt.Printf("LGM : Failed to delete link %s: %v\n", link_svi, err)
                return false
        }
