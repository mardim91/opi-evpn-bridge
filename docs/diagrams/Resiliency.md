...
The architecture of the opi-evpn-br has been designed with resiliency and data persistency in mind. In the below diagram is descibed how the system recovers after a power cut event. The introduced resiliency is expressed by having a persistent database where all the opi-evpn-br related objects are retained. After a power cut event and when the system recovers the opi-evpn-br will replay everything that is stored in its persistent database targeting to bring the underlying slowpath and fastpath configuration in its former state that was before the powercut. The example below is focused on a VRF replay scenario for simplicity but this can be extended to all the objects that are stored in the database.
...
rect rgb(255, 229, 204)
    ...
    DB ->> InfraDB: Get VRF object from the Database
    InfraDB ->> InfraDB: Change overall VRF status to Down and all the Modules status to Pending
    InfraDB ->> DB: Store the VRF object back to the DB
    InfraDB ->> TaskManager: Create a Task to realize the VRF object intent 
end
